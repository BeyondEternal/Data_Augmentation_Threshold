...
library(reshape2)
#####Loading data set#############
datasets <- c( "Maize_AL", "Wheat_1_AL", 
...
for(dataset in datasets){
  ...
  #   i=1
        ...
        ETA <- list(list(K = GenoLine, model = "RKHS"))
           ...
           Optimal_Threshold=length(which(yy[-testing_indices]==1))/length(yy[-testing_indices])
  ...
  write.csv(Accuracy_A_Val,file=file.path(results_dir,"Accuracy_A_Optimal_Threshold.csv"))
  write.csv(Predictions,file=file.path(results_dir,"ALL_Predictions_Optimal_Threshold.csv"))
} 
