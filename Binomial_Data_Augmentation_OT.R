...
for(dataset in datasets){
  ...
  # trait=Traits_to_evaluate[1]
    ...
    #   i=1
        ...
        length(Lines_Fold_i)
        Pheno_df <-Pheno_E %>% slice(rep(validation_indices, times = 1))
        dim(Pheno_df)
        Pheno_df=droplevels(Pheno_df)
        rownames(Pheno_df)=(nrow(Pheno)+1):(nrow(Pheno)+nrow(Pheno_df))
      
        Pheno_df_trn=Pheno_E[-validation_indices,]
        Pheno_df_trn=droplevels(Pheno_df_trn)
        dim(Pheno_df_trn)
        dim(Pheno_E)
        dim(Pheno_df)
        ...
        y_comb <- Pheno_df
        index_comb_rows <- combn(nrow(y_comb), 2)
          
        Pheno_Aug<-c(as.numeric(y_Bin_Factor[fold$testing])-1,as.numeric(y_Bin_Factor[fold$training])-1,rep(1,ncol(index_comb_rows)))
        Pheno_Aug
        #Este almacena solo el testing y el 20% mejor
        Pheno_Aug_df <- rbind(Pheno_E[fold$testing,], Pheno_E[fold$training,])
        dim(Pheno_Aug_df)
        Trait_Bin=colnames(Pheno_E)
        Trait_Bin=Trait_Bin[length(Trait_Bin)]
   
        Pheno_Aug_df =droplevels(  Pheno_Aug_df )
        dim(Pheno_Aug_df)
        
        Line <- model.matrix(~ 0 + Line, data = Pheno_Aug_df)
        length(Pheno_Aug)
        lines <- sort(Pheno_Aug_df$Line)
        Geno2 <- Geno[lines,lines]
        dim(Geno2)
      
        
        LineBinded <- matrix(nrow= 0, ncol = ncol(Line))
        for(index in 1:ncol(index_comb_rows)){
          i <- index_comb_rows[1, index]
          j <- index_comb_rows[2, index]
          bindLine <- matrix(0, nrow=1, ncol = ncol(Line))
          bindLine[i]<-lambda
          bindLine[j]<-1-lambda
          LineBinded <- rbind(LineBinded, bindLine)
        }
        #Aquí se agregan las nuevas lineas hibridas
        Line <- rbind(Line, LineBinded)
        dim(Line)
        dim(Geno2)
        yy<-as.factor(Pheno_Aug)
        #Se seleccionan las testing agregadas al Pheno_Aug (son las primeras puesto que se agregan al principio)
        testing_indices_final=which(y_Bin_Factor[fold$testing] %in% Pheno_Aug)
        GenoLine <- Line %*% Geno2 %*% t(Line)
        dim(Line)
        y_na <- replace(yy, c(testing_indices_final), NA)
        ETA <- list(list(K = GenoLine, model = "RKHS"))
           ...
           validation_predictions <- model$probs[testing_indices_final,2]
           validation_observed <- yy[testing_indices_final]
           validation_Bin_observed <- yy[testing_indices_final]
           Top_Threshold=length(which(yy[-testing_indices_final]==1))/length(yy[-testing_indices_final])
           Optimal_Threshold=length(which(y_Bin_Factor[-testing_indices]==1))/length(y_Bin_Factor[-testing_indices])
    
           Pred_Bin_validation=ifelse( validation_predictions>Optimal_Threshold,1,0)
        ...
        Predictions <- rbind(
           ...
           data.frame(
              ...
              Opt_Threshold_Clasif=Optimal_Threshold
  ...
  write.csv(Summary_final,file=file.path(results_dir,"Accuracy_Data_Augmentation_OT.csv"))
  write.csv(Predictions_final,file=file.path(results_dir,"ALL_Predictions_Data_Augmentation_OT.csv"))
}
